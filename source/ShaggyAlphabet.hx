 ...
 */
class ShaggyAlphabet extends FlxSpriteGroup
{
	...
	var lastSprite:ShaggyCharacter;
	var xPosResetted:Bool = false;

	var splitWords:Array<String> = [];

	var isBold:Bool = false;
	public var lettersArray:Array<ShaggyCharacter> = [];
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isNumber:Bool = ShaggyCharacter.numbers.indexOf(character) != -1;
			var isSymbol:Bool = ShaggyCharacter.symbols.indexOf(character) != -1;
			var isAlphabet:Bool = ShaggyCharacter.alphabet.indexOf(character.toLowerCase()) != -1;
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:ShaggyCharacter = new ShaggyCharacter(30 * loopNum, 0, textSize);
				var letter:ShaggyCharacter = new ShaggyCharacter(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length) {
			...
			var isNumber:Bool = ShaggyCharacter.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = ShaggyCharacter.symbols.indexOf(splitWords[loopNum]) != -1;
			var isAlphabet:Bool = ShaggyCharacter.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:ShaggyCharacter = new ShaggyCharacter(30 * loopNum, 0, textSize);
				var letter:ShaggyCharacter = new ShaggyCharacter(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class ShaggyCharacter extends FlxSprite
{
