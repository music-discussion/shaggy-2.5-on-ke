...
class DialogueBoxPsych extends FlxSpriteGroup
{
		...
		if(!dialogueEnded) {
			...
			if(FlxG.keys.justPressed.ANY) {
				if(!daText.finishedText) {
					...
					daText = new PsychAlphabet(0, 0, textToType, false, true, 0.0, 0.7);
					daText.x = textX;
					daText.y = textY;
					add(daText);
	...
	function startNextDialog():Void
	{
		...
		daText = new PsychAlphabet(textX, textY, textToType, false, true, speed, 0.7);
		add(daText);
