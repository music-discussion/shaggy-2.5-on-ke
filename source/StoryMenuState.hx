...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...CoolUtil.storyStuff.length) {
			var sprDifficulty:FlxSprite = new FlxSprite(leftArrow.x + 35, leftArrow.y).loadGraphic(Paths.image('menudifficulties/' + CoolUtil.storyStuff[i][0].toLowerCase()));
			...
			switch (curWeek)
			{
				...
				default:
					...
					if (curDifficulty != 0 || WeekData.maniaSongs[curWeek][0] != '')
					{
						...
						var diffic = CoolUtil.storyStuff[curDifficulty][1];
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		if (curDifficulty < 0)
			curDifficulty = CoolUtil.storyStuff.length-1;
		if (curDifficulty >= CoolUtil.storyStuff.length)
			curDifficulty = 0;
