...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Shaggy burst', "No values. Just place and watch the burst appear around Shaggy."],
		['Camera rotate on', "Value 1: Speed multiplier\nValue 2: Range"],
		['Camera rotate off', "Self explandatory?"],
		['Toggle bg dim', "Simple enough, huh?"],
		['Drop eye', "to whoever is editing this chart:\ndon't use this anywhere.\nNote from Discussions: For as long as I was ripping code. I'm pretty sure this is the part that drops off of\nWBShaggy in \"hell\". So I recommend to not use this as it can cause some serious damage."],
		['Psyshock', ":)"],
		['Fakeshock', ":)"],
	...
	var gridBG:FlxSprite; //used for mania shit and events?? I'm not too experienced.
	...
	**/
	...
	public var sectionDtype:Array<Int>;
	...
	{
		/*if (stepperDType.value != lastValue)
		{
			stepperDType.value = lastValue;
			stepperDType.value = _song.dType[curSection];
		}*/
		...
		if (FlxG.keys.justPressed.F11)
		{
			if(!FlxG.keys.pressed.SHIFT)
				...
				trace(_song.dType[curSection]);
			}
		}
		if(FlxG.keys.justPressed.F12)
			{
				_song.dType[curSection] = Std.int(stepperDType.value);
			}
			...
			if (FlxG.mouse.overlaps(curRenderedNotes))
			{
				curRenderedNotes.forEach(function(note:Note)
				{
					if (FlxG.mouse.overlaps(note))
					{
						...
						{
						//	if (pressArray[Math.floor(Math.abs(note.noteData))]) {
	...
	{
		if (check_mustHitSection.checked) //oh yea gotta use psych's "changeIcon" function
		{
	...
	{
		/*try {
		//if (_song.dType[curSection] > 0 || _song.dType[curSection] == 1 || _song.dType[curSection] == 2 || _song.dType[curSection] == 3 || _song.dType[curSection] == 4 || _song.dType[curSection] == 5) {
		_song.dType.push(0);
		return 0;
	//	}
		} catch (exception)
		{*/
		//	Application.current.window.alert("Something went wrong. To prevent the game crashing, we will just send you back to Playstate. Or at least it will try. Error: " + exception);
			return 0; //all we need.
	...
	private function addSection(lengthInSteps:Int = 16):Void 
	{
		var sec:SwagSection = {
			...
			typeOfSection: 0,
			altAnim: false,
			dType: dTypePush() //what this does is it pushes a new 0 while also returning 0 too :), so that way when you edit it, it has something to change instead of it being null
		};

		//sec.dType = sectionDtype;
	...
	private function addNote(?n:Note):Void
	{
		...
		if (_song.dType != sectionDtype)
		{
			trace('something is wrong');
			sectionDtype = _song.dType;
		}
